...
#########################################################################
# function dispatch table for compiled functions
dispatch_table = {}

#########################################################################
def set_lineinfo(module_name, lineno):
    state.lineinfo = (module_name, lineno)
...
def handle_call(obj_ref, fval, actual_val_args, fname):
    ...
    (FUNCTION_VAL, (IMPLEMENTATION, fpointer), closure) = fval
    ...
    return_value = dispatch_table[fpointer](actual_val_args)
    if not return_value:
