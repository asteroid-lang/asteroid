...
and a variable as pattern.  In that case the variable is simply instantiated with the value.
...
Notice that patterns allowed to precisely define what we mean by positive or negative integers in one place and then use these patterns in our function.
...
on literals is convenient for statement like these,
::
   let (1,x) = p.

This ``let`` statement is only successful for values of ``p`` which are pairs where the first component is the pair is the value ``1``.
...
**Think of structures in Asteroid as classes in C++, and in both languages these allow you to instantiate objects** (that means if you have programmed with classes and objects in C++, creating structures in Asteroid should be trivial). Something else to note is that similar to Rust and Go, **Asteroid does not have inheritence for classes**.  That is why programming in Asteroid is sometimes referred to as object-based programming rather than object-oriented programming.
...
The name of the class above can now be considered a user defined data type and can appear whereever built-in data type names can appear.
