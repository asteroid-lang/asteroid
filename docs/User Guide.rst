...
Asteroid supports two more built-in data types:
...
The above factorial program can be simplified by rewriting the first condition on ``n`` as a named pattern and also taking advantage of the fact that the last expression evaluated in a function body provides an implicit return value gives us,
::
    load system io.
